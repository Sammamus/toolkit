...
var slugTests = []struct {
	...
	{name: "japanese string", s: "こんにちは世界", expected: "now-is-the-time", errorExpected: true},
...
func TestTools_Slugify(t *testing.T) {
	...
	for _, e := range slugTests {
		...
		if !e.errorExpected && slug != e.expected {
			t.Errorf("%s wrong slug returned; expected %s bug got %s", e.name, e.expected, slug)
		}
	}
}
